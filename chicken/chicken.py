...
new_start = 1
have_got_size = False
higest_score = float(0)
def start_game():
    global higest_score,fullscreen,screen_width,screen_height,have_got_size,screen, screen_rect, chicken_rect, upper_group, fall_egg_image,up_egg_image,small_chicken2_left_image,small_chicken2_right_image,new_start,egg_upper_list,move_egg
    # 设置定时器
    if not difficult:
        interval = 500
    else:
    ...
    while 1:
        while going:
            ...
            txt1 = txt_font.render(f'highest:{higest_score}',True,'red','yellow')
            txt1_rect = txt1.get_rect()
            txt1_rect.topleft = txt_rect.bottomleft
            ...
            if score > higest_score:#获取最大分数记录
                higest_score = score
